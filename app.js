...
function defaultState() {
  return {
    ...
    settings: {
      ...
      honorific: 'さん',
      bodyWeightKg: null,
      mascotEnabled: true,          // キャラクター表示のON/OFF
      activeMascotSetId: 'set_default', // 現在選択されているセットのID
      mascotSets: [
        {
          id: 'set_default',
          name: 'デフォルト',
  ...
  try {
    ...
    if (raw) {
      try {
        ...
        // 旧仕様(mascotImages)から新仕様(mascotSets)へのデータ自動移行
...
function showMascot(expression, message, autoHide = true) {
  if (!state.settings.mascotEnabled) return; // オフの場合は表示しない
...
function renderWorkout() {
  ...
  const logRows = logs.map((l, i) => {
    ...
    return `<div class="list-row"><span class="sub">セット${i + 1}</span><span class="val">${parts.join(' × ')}${kcalText}</span></div>`;
  }).join('');
  ...
  const workoutRows = workoutLogs.map(l => {
    ...
    return `
      <div class="list-row">
        ...
        <button class="delete-btn" data-delete-workout-log="${l.id}"><i class="ti ti-trash"></i></button>
      </div>
    `;
  }).join('');
  ...
  const mealRows = meals.map(m => `
    <div class="list-row">
      ...
      <span style="display:flex; align-items:center; gap:10px;">
        <span class="val gold">${Number(m.calories).toLocaleString()} kcal</span>
        <button class="delete-btn" data-delete-meal="${m.id}"><i class="ti ti-trash"></i></button>
      </span>
    </div>
  `).join('');
    ...
    return `
      <div class="list-row">
        ...
        <span class="sub" style="display:flex; align-items:center; gap:10px;">
          ${tags.join('・') || 'なし'}
          <button class="delete-btn" data-delete-exercise="${e.id}"><i class="ti ti-trash"></i></button>
        </span>
      </div>
    `;
  }).join('');
